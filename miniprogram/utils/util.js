...
//获取 formId 缓存日期
const isUpdateFormId = () => {
  let now = moment().format('YYYY/MM/DD');
  let formIdDate = wx.getStorageSync('formIdDate');

  if (formIdDate != null && formIdDate == now){
    return false;
  }else{
    return true;
  }
...
//获取 formId 缓存日期
const updateFormId = () => {
  let formIdDate = moment().format('YYYY/MM/DD');
  wx.setStorageSync('formIdDate', formIdDate)
}
